...
std::string BookingsHandler::HandleRequestThrow(
  ...
  try {
    ...
    const auto rate_limit = service_.CheckBookingCreationRateLimit(auth.user_id);
    request.GetHttpResponse().SetHeader(std::string("X-RateLimit-Limit"), std::to_string(rate_limit.limit));
