...
HotelBookingService::HotelBookingService(
    ...
    : pg_cluster_(std::move(pg_cluster)),
      ...
      redis_cc_(std::chrono::seconds{3}, std::chrono::seconds{3}, 2) {}
...
std::vector<Hotel> HotelBookingService::ListHotels(const std::optional<std::string>& city) const {
  const auto cache_key = BuildHotelsCacheKey(city);
  const auto now = Clock::now();
  {
    std::lock_guard<std::mutex> lock(mutex_);
    const auto cache_it = hotels_cache_.find(cache_key);
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    hotels_cache_[cache_key] = CachedHotelsEntry{hotels, now + std::chrono::seconds(60)};
  }
...
std::vector<Booking> HotelBookingService::ListUserBookings(
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    const auto cache_it = user_bookings_cache_.find(target_user_id);
  ...
  {
    std::lock_guard<std::mutex> lock(mutex_);
    user_bookings_cache_[target_user_id] =
        CachedUserBookingsEntry{result, now + std::chrono::seconds(30)};
  }
...
void HotelBookingService::CancelBooking(std::int64_t requester_user_id, std::int64_t booking_id) {
  ...
  InvalidateUserBookingsCache(requester_user_id);
}

HotelBookingService::RateLimitDecision HotelBookingService::CheckBookingCreationRateLimit(
    std::int64_t user_id) {
  constexpr std::uint32_t kRequestLimit = 20;
  constexpr std::int64_t kWindowSeconds = 60;
  constexpr std::int64_t kTtlSeconds = 70;

  const auto now_seconds = std::chrono::duration_cast<std::chrono::seconds>(
      std::chrono::system_clock::now().time_since_epoch()).count();
  const auto window_id = now_seconds / kWindowSeconds;
  const auto reset_seconds = (window_id + 1) * kWindowSeconds;

  const auto redis_key = "rate_limit:bookings:user:" + std::to_string(user_id) +
      ":window:" + std::to_string(window_id);
  const auto current_count = redis_client_->Incr(redis_key, redis_cc_).Get();
  if (current_count == 1) {
    redis_client_->Expire(redis_key, std::chrono::seconds{kTtlSeconds}, redis_cc_).Get();
  }

  if (current_count > static_cast<std::int64_t>(kRequestLimit)) {
    return RateLimitDecision{false, kRequestLimit, 0, reset_seconds};
  }

  return RateLimitDecision{
      true,
      kRequestLimit,
      static_cast<std::uint32_t>(kRequestLimit - current_count),
      reset_seconds};
}

void HotelBookingService::InvalidateHotelsCache() {
  std::lock_guard<std::mutex> lock(mutex_);
  hotels_cache_.clear();
}

void HotelBookingService::InvalidateUserBookingsCache(std::int64_t user_id) {
  std::lock_guard<std::mutex> lock(mutex_);
  user_bookings_cache_.erase(user_id);
}

std::string HotelBookingService::BuildHotelsCacheKey(const std::optional<std::string>& city_filter) {
  if (!city_filter.has_value()) {
    return "all";
  }
  const auto city_value = Trim(*city_filter);
  return city_value.empty() ? "all" : "city:" + Normalize(city_value);
}
